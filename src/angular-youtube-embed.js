    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        if (startWithImage === true) {
          iframeEl = angular.element('<div></div>');
          var wrapper = $compile(angular.element('<div ng-show="imageClicked"></div>'))(scope);
          var imageWrapper = $compile(angular.element(
            '<div class="image-wrapper" ng-hide="imageClicked" ng-click="imageClick()">' +
            '<img ng-src="http://img.youtube.com/vi/{{videoId}}/0.jpg" ng-style="{ \'height\': playerHeight + \'px\' }">' +
            '<div></div>' +
            '</div>'))(scope);

          wrapper.append(iframeEl)
